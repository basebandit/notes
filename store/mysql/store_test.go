...
func getTestStore(t *testing.T) (*Store, func()) {
	...
	if err != nil {
		t.Fatalf("failed to connect to the test mysql database: address=%q, username=%q, password=%q, database=%q: %s", testAddress, testUsername, testPassword, testDatabase, err)
	}
